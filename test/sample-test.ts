...
describe("Niji", async () => {
  ...
  it("コンテンツidがちゃんとインクリメントされるか", async () => {
    await assert((await niji.functions.getNumberOfContents()).toString(), "1");
    await niji.connect(author).functions.registerOriginal("10");
    await assert((await niji.functions.getNumberOfContents()).toString(), "2");
    await niji.connect(author2).functions.registerSecondary(1, {value: 10});
  })

  it("コンテンツリストはちゃんと機能しているのか", async () => {
    await niji.connect(author).functions.registerOriginal("0");
    await niji.connect(author2).functions.registerOriginal("0");
    await niji.connect(author).functions.registerOriginal("0");
    await niji.connect(author2).functions.registerOriginal("0");
    await niji.connect(author).functions.registerOriginal("0");

    let list1 = await niji.functions.getContentsList(await author.getAddress());
    ...
    let list2 = await niji.functions.getContentsList(await author2.getAddress());
  ...
  it("権利の確認と剥奪や付与", async () => {
    await niji.connect(author).functions.registerOriginal("10");
    await niji.connect(author2).functions.registerSecondary(1, {value: 10});
    assert.equal((await niji.functions.checkRight(2))[0], true);
    assert.equal((await niji.functions.getParent(2)).toString(), "1");
    // 剥奪は一次作者以外はできない
    await assertPromiseThrow(niji.connect(other).functions.deprive(1, 2));
    assert.equal((await niji.functions.checkRight(2))[0], true);
    //剥奪する
    await niji.connect(author).functions.deprive(1, 2);
    assert.equal((await niji.functions.checkRight(2))[0], false);
    //権利回復も一次作者以外はできない
    await assertPromiseThrow(niji.connect(other).functions.recover(1, 2));
    assert.equal((await niji.functions.checkRight(2))[0], false);
    //権利回復
    await niji.connect(author).functions.recover(1, 2);
    assert.equal((await niji.functions.checkRight(2))[0], true);
  })
})
